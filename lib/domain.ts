...
export type CartLine = { ean: string; nombre: string; precioVenta: number; qty: number; unidadVenta?: SaleUnit; custom_amount?: true };
export const isWeight = (item: { unidadVenta?: SaleUnit }) => item.unidadVenta === "peso";
export const quantityLabel = (qty: number, item: { unidadVenta?: SaleUnit }) => isWeight(item) ? `${new Intl.NumberFormat("es-AR", { maximumFractionDigits: 3 }).format(qty / 1000)} kg` : `${qty} unidades`;
export function kilogramsToGrams(kg: number): number {
  const grams = Math.round(kg * 1000);
  if (!Number.isFinite(kg) || kg < 0 || !Number.isSafeInteger(grams) || Math.abs(kg * 1000 - grams) > 1e-6) throw new Error("Ingresá kilos con hasta tres decimales.");
  return grams;
}
export const lineCents = (line: CartLine) => Math.round(cents(line.precioVenta) * line.qty / (isWeight(line) ? 1000 : 1));
...
export function validateProduct(product: Product) {
  if (product.unidadVenta !== undefined && !["unidad", "peso"].includes(product.unidadVenta)) throw new Error("Tipo de venta inválido.");
