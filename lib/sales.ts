  ...
  try {
    ...
    for (const line of lines) {
      if (line.custom_amount) {
        ...
        confirmed.push({ ean: line.ean, nombre: "Importe libre", precioVenta: cents(line.precioVenta) / 100, qty: line.qty, custom_amount: true });
        continue;
      }
...
export function startOfToday(now = new Date()) { const date = new Date(now); date.setHours(0, 0, 0, 0); return date.getTime(); }
