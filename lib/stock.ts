...
export type WasteRecord = { id: string; ean: string; nombre: string; grams: number; reason: string; createdAt: number };
export async function recordWaste(id: string, ean: string, grams: number, reason: string): Promise<Product> {
  if (!id || !Number.isSafeInteger(grams) || grams <= 0 || !reason.trim() || reason.trim().length > 120) throw new Error("Ingresá gramos enteros mayores a cero y un motivo de hasta 120 caracteres.");
  ...
  try {
    ...
    if (!current || current.unidadVenta !== "peso") throw new Error("Seleccioná un producto vendido por peso.");
    if (previous) {
      if (previous.ean !== ean || previous.grams !== grams || previous.reason !== reason.trim()) throw new Error("El registro de merma ya existe con otros datos.");
      await done; return current;
    }
    if (grams > current.stock) throw new Error("La merma supera el stock disponible.");
    const updated = { ...current, stock: current.stock - grams, updatedAt: new Date().toISOString() };
    validateProduct(updated);
    products.put(updated);
    waste.add({ id, ean, nombre: current.nombre, grams, reason: reason.trim(), createdAt: Date.now() } satisfies WasteRecord);
