...
test("peso: precisión, venta mixta, reposición y merma atómicas", async () => {
  const ham: Product = { ean: "ham", nombre: "Jamón", unidadVenta: "peso", costo: 6000, margen: 66.7, precioVenta: 10000, stock: 0, updatedAt: "" };
  const unit: Product = { ...ham, ean: "bread", nombre: "Pan", unidadVenta: undefined, precioVenta: 150 };
  assert.equal(kilogramsToGrams(8.250), 8250);
  ...
  const lines = [{ ...ham, qty: 235 }, { ...unit, qty: 2 }];
  ...
  await assert.rejects(checkout("wrong-unit", [{ ...ham, unidadVenta: undefined, qty: 1 }], "EFECTIVO"));
  ...
  assert.equal((await requestValue(db.transaction("waste").objectStore("waste").getAll())).length, 1);
  db.close();
