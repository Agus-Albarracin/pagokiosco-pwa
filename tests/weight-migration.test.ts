...
test("la migración v1 a v2 conserva productos y ventas por unidad", async () => {
  const legacy = await new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      request.result.createObjectStore("sales", { keyPath: "id" }).createIndex("createdAt", "createdAt");
    };
    request.onsuccess = () => resolve(request.result);
    request.onerror = () => reject(request.error);
  });
  const product = { ean: "old", nombre: "Galletitas", costo: 100, margen: 50, precioVenta: 150, stock: 3, updatedAt: "" };
  const tx = legacy.transaction(["products", "sales"], "readwrite");
  const done = transactionDone(tx);
  tx.objectStore("products").put(product);
  tx.objectStore("sales").put({ id: "old-sale", createdAt: 1, method: "EFECTIVO", lines: [{ ...product, qty: 1 }], total: 150 });
  await done; legacy.close();
  assert.deepEqual(await listProducts(), [product]);
  const db = await openDatabase();
  assert.equal(db.version, 2); assert.equal(db.objectStoreNames.contains("waste"), true); db.close();
