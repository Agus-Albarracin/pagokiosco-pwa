...
test("catálogo y reposición separados conservan precios y carrito al navegar", async ({
  page,
}, testInfo) => {
  ...
  await page.screenshot({
    path: "test-results/stock-" + testInfo.project.name + ".png",
    fullPage: true,
  });
  ...
  await expect(page.locator("main button")).toHaveCount(0);
  await page.screenshot({
    path: "test-results/catalog-" + testInfo.project.name + ".png",
    fullPage: true,
  });
  await nav.getByRole("button", { name: "Agregar stock", exact: true }).click();
  ...
  await expect(page.getByRole("dialog")).toContainText(
    "Para reponer, usá la sección Agregar stock.",
  );
  ...
  await page.getByLabel("Buscar producto para reponer").fill("manaos");
  await page
    .getByRole("button", { name: "Agregar stock a Manaos pomelo blanco zero", exact: true })
    .click();
...
test("la navegación móvil permite deslizar en ambos sentidos sin desbordar la página", async ({
  page,
  context,
  isMobile,
}) => {
  ...
  await nav.getByRole("button", { name: "Caja", exact: true }).click();
  await expect(nav.getByRole("button", { name: "Caja", exact: true })).toHaveAttribute(
    "aria-current",
    "page",
  );
