...
export function WorkspaceNav({ view, onChange }: {
  view: WorkspaceView; onChange: (view: WorkspaceView) => void;
}) {
  ...
  return <>
    <nav ref={nav} className="workspace-nav" aria-label="Principal">
      {sections.map(section => <button key={section.view}
    ...
    <p className="nav-scroll-hint">Deslizá las secciones <span aria-hidden="true">↔</span></p>
  </>;
}
