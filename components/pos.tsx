...
import { isWeight, lineCents, quantityLabel, money, type PaymentMethod, type Product } from "@/lib/domain";
...
export function Pos({ products, onRefresh, disabled }: { products: Product[]; onRefresh: () => Promise<void>; disabled: boolean }) {
  ...
  return <><div className="pos-layout"><section className="panel"><div className="section-head"><h2>Elegí los productos</h2></div><label className="search-label"><span>Buscar para vender</span><input type="search" placeholder="Buscar por marca o producto…" value={query} onChange={e => setQuery(e.target.value)} disabled={disabled} /></label>
    <div className="quick-grid">{filtered.map(p => <button key={p.ean} className="quick-product" disabled={disabled || p.stock <= (cart.lines.find(l => l.ean === p.ean)?.qty ?? 0)} onClick={() => { if (isWeight(p)) setWeight({ product: p, editing: false }); else cart.add(p); setSuccess(""); }}><span className="quick-code">{p.marca || "PRODUCTO"}</span><strong>{p.nombre}</strong><span>{money(p.precioVenta)}{isWeight(p) ? " /kg" : ""}</span><small>{p.stock > 0 ? `${quantityLabel(p.stock, p)} disponibles` : "Sin stock"}</small></button>)}</div>
  ...
  </section><section className="panel cart-panel"><div className="section-head"><h2>Venta actual</h2><span className="pill">{cart.lines.length} productos</span></div>
    {!cart.lines.length ? <div className="empty"><span className="empty-symbol">＋</span><h3>Empezá una nueva venta</h3><p>Los productos que elijas aparecen acá.</p></div> : <ul className="cart-lines">{cart.lines.map(line => <li key={line.ean}><div className="cart-line-title"><span>{isWeight(line) ? line.qty + " g" : line.qty} × {line.nombre}</span><strong>{money(lineCents(line) / 100)}</strong></div><div className={isWeight(line) ? "quantity weight-quantity" : "quantity"}>{isWeight(line) ? <><button disabled={busy} aria-label={"Cambiar peso de " + line.nombre} onClick={() => { const product = products.find(p => p.ean === line.ean); if (product) setWeight({ product, editing: true }); }}>Cambiar peso</button><button disabled={busy} aria-label={"Quitar " + line.nombre} onClick={() => cart.change(line.ean, -line.qty)}>Quitar</button></> : <><button aria-label={`Quitar una unidad de ${line.nombre}`} disabled={busy} onClick={() => cart.change(line.ean, -1)}>−</button><span>{line.qty}</span><button aria-label={`Agregar una unidad de ${line.nombre}`} disabled={busy || (!line.custom_amount && line.qty >= (products.find(p => p.ean === line.ean)?.stock ?? 0))} onClick={() => cart.change(line.ean, 1)}>＋</button></>}<small>{money(line.precioVenta)} {isWeight(line) ? "/kg" : "c/u"}</small></div></li>)}</ul>}
  ...
  {weight && <WeightPicker product={weight.product} available={weight.product.stock - (weight.editing ? 0 : (cart.lines.find(line => line.ean === weight.product.ean)?.qty ?? 0))} initial={weight.editing ? cart.lines.find(line => line.ean === weight.product.ean)?.qty : Math.min(100, weight.product.stock - (cart.lines.find(line => line.ean === weight.product.ean)?.qty ?? 0))} onClose={() => setWeight(null)} onConfirm={grams => { if (weight.editing) cart.change(weight.product.ean, grams - (cart.lines.find(line => line.ean === weight.product.ean)?.qty ?? 0)); else cart.add(weight.product, grams); }} />}
