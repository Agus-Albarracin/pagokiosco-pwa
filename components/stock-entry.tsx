...
export function StockEntry({ products, disabled, onCreate, onSelect, onEdit, onWaste }: {
  products: Product[]; disabled: boolean; onCreate: () => void;
  onSelect: (product: Product) => void;
  onEdit: (product: Product) => void; onWaste: (product: Product) => void;
}) {
  ...
  return <div className="stock-entry">
    <section className="panel" aria-labelledby="stock-search-title">
      ...
      <p className="muted stock-search-description">Encontrá un producto por marca o nombre y elegí qué cantidad sumar.</p>
      <label className="search-label">Buscar producto para reponer
        ...
        <div className="product-list">{matches.map(product => <div className="product-row" key={product.ean}>
          <div className="product-name"><strong>{product.nombre}</strong>{product.marca && <small>{product.marca}</small>}<small>Stock actual: {quantityLabel(product.stock, product)}</small></div>
        ...
        {isWeight(product) && <button className="quiet" aria-label={"Registrar merma de " + product.nombre} disabled={disabled || product.stock <= 0} onClick={() => onWaste(product)}>Registrar merma</button>}</div>)}</div>
